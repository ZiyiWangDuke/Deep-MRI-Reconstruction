...
img_256 = np.load('dl_research/projects/under_recon/single_channel_pad.npy')
# shape (8, 7, 256, 256, 36)

img_out = np.zeros(img_256.shape)
zero_filling = np.zeros(img_256.shape)
img_full = np.zeros(img_256.shape)
...
for k in range(img_256.shape[0]): # iterate through coils
    for m in range(img_256.shape[1]): # iterate through contrast

        input_ims = img_256[k,m,:,:,:]

        input_ims = np.transpose(input_ims,(2,0,1))
        input_ims = np.stack((input_ims,np.zeros(input_ims.shape)),axis=-1)

        ims_sample, masks, k_sample, down_rate = down_sample_with_mask(input_ims)
...
if key == 'comb_contrast':
    for k in range(img_256.shape[0]):
        # coil 
        for m in range(img_256.shape[4]):
        # location
...
if key == 'comb_contrast_coil':
    plt_im_sample_c = np.zeros((img_256.shape[0],img_256.shape[2],img_256.shape[3]))
    plt_im_predict_c = np.zeros((img_256.shape[0],img_256.shape[2],img_256.shape[3]))
    plt_full_c = np.zeros((img_256.shape[0],img_256.shape[2],img_256.shape[3]))
    ...
    for m in range(img_256.shape[4]):
        ...
        plt.savefig('output/figures/predict_hp_unet_single_slice_'+str(m))
        plt.close()
