...
# RUN_LABEL="unet_full" USERNAME=zwang ./qsub-run.sh  python -m dl_research.projects.under_recon.Train_hp_datagen_cord
...
from .utils_functions import var_2d_mask, down_sample_with_mask, relative_error_center_30pix, relative_error_center_30pix_mp, loss_weight_mse
from .utils_keras import TensorBoardImage
from .model_Full_Basic import recon_encoder
from .data_gen import get_data_gen_model_cord
...
batch_iterator_train = get_data_gen_model_cord(data_type=data_train, batch_size=batch_size, epoch_size=epoch_size, flag='train')
batch_iterator_valid = get_data_gen_model_cord(data_type=data_valid, batch_size=batch_size, epoch_size=epoch_size, flag='validation')
...
tensor_log_dir = "output/keras_logs/{}_unet_phase_full/".format(datetime.strftime(datetime.now(), '%Y_%m_%d_%H_%M'))
...
recon_encoder.save_weights('output/models/under_recon_180702_unet_phase_full.h5')
